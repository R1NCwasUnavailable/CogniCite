...
app = FastAPI(
    title="CogniCite — Research Paper Assistant API",
...
MAX_HISTORY_MESSAGES = 20     # Trim chat history to last N messages to avoid token overflow
MAX_CONTEXT_CHARS = 8000      # Cap recalled context to prevent exceeding LLM token limits
RECALL_TIMEOUT_SECS = 15      # Max seconds to wait for Cognee recall before falling back
...
async def upload_paper(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    try:
        ...
        metadata = await asyncio.to_thread(
            extract_pdf_metadata_and_claims, text, existing
        )

        # 5. Check for duplicate title
        title = metadata.get("title", file.filename)
        if paper_title_exists(title):
            raise HTTPException(
                status_code=409,
                detail=f"A paper titled \"{title}\" already exists in the library."
            )

        # 6. Generate a unique ID for this paper
        ...
        # 7. Add paper to SQL DB (sets status badge, timeline, and graph info)
        add_paper_to_db(
            paper_id=paper_id,
            title=title,
            authors=authors,
        ...
        # 8. Index paper text into Cognee in the background (prevent blocking endpoint)
...
async def chat_session(request: ChatRequest):
    ...
    try:
        # 1. Recall semantic context from Cognee (with timeout fallback)
        try:
            context = await asyncio.wait_for(
                ...
                timeout=RECALL_TIMEOUT_SECS
            )
        except asyncio.TimeoutError:
            print(f"Cognee recall timed out after {RECALL_TIMEOUT_SECS}s, proceeding with empty context")
            context = ""

        # Cap context length to avoid LLM token overflow
        if len(context) > MAX_CONTEXT_CHARS:
            context = context[:MAX_CONTEXT_CHARS] + "\n\n[Context truncated for length]"

        # 2. Convert & trim message history
        history_list = [
            {"role": msg.role, "content": msg.content}
            for msg in request.history[-MAX_HISTORY_MESSAGES:]
        ]

        # 3. Generate response using Groq LLM wrapper (run in thread to avoid blocking)
        ...
        # 4. Analyze reply for contradictions & claim counts (run in thread)
...
async def export_review(request: ExportRequest):
    ...
    try:
        ...
        # Recall context with timeout
        try:
            context = await asyncio.wait_for(
                recall_context(query=summary_query, query_mode="cross-paper"),
                timeout=RECALL_TIMEOUT_SECS
            )
        except asyncio.TimeoutError:
            context = ""
        ...
        # Generate markdown via LLM (run in thread)
        markdown_str = await asyncio.to_thread(generate_literature_review, context)
